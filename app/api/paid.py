...
def check():
    ...
    for user in User:
        if user.last_paid + duration > datetime.now(timezone.utc):
            user.paid = False
            res = transaction.charge(user.card['email'], auth_code=user.card['authorization_code'], amount=190233)
...
def paid():
    ...
    user.card = data['data']['authorization']
    print(data['data']['authorization']['email'])
    print(data['event'])
    user.paid = True
